...
class PGRCNNHead(RoIHeadTemplatePG):
    ...
    def roi_grid_pool(self, batch_dict, rois, grid_size):
        """
        Args:
            batch_dict:
                batch_size:
    ...
    def forward(self, batch_dict):
        ...
        for b, batch_roi in enumerate(roi):
            for value in batch_roi:
                if value[3].item() * value[4].item() * value[5].item() < self.pool_cfg.VOLUME_THRESHOLD:
                    grid_size = grid_sizes[0]
                else:
                    grid_size = grid_sizes[1]
                pooled_features, roi_grid_xyz_rel = self.roi_grid_pool(batch_dict, value.view(1, 1, value.shape[0]), grid_size)  # (BxN, 6x6x6, C) pooled_features: (B, 6x6x6, 96), roi_grid_xyz_rel(B, 6x6x6, 3)
                ...
                # Transformer Encoder
                positional_input, global_roi_grid_points = self.get_positional_input(value.view(1, 1, value.shape[0]), batch_dict['roi_labels'][b], roi_grid_xyz_rel)
