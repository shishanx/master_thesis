...
class PGRCNNHead(RoIHeadTemplatePG):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        # Transformer Encoder
        assert self.attention_cfg.ENCODER.NUM_FEATURES == c_out_1, f'ATTENTION.ENCODER.NUM_FEATURES must equal voxel aggregation output dimension of {c_out}.'
        ...
        for k in range(0, self.model_cfg.GEN_FC.__len__()):
            ...
            if k != self.model_cfg.GEN_FC.__len__() - 1 and self.model_cfg.DP_RATIO > 0:
                gen_fc_list.append(nn.Dropout(self.model_cfg.DP_RATIO))
        gen_fc_list.append(nn.Linear(c_out, (3 + self.point_cfg.POINT_FEATURE_NUM) * self.point_cfg.NUM_POINTS, bias=True))
        self.gen_fc_layers_1= nn.Sequential(*gen_fc_list)
    ...
    def init_weights(self):
        ...
        fc_list = [self.gen_fc_layers, self.gen_fc_layers_1, self.cls_fc_layers, self.reg_fc_layers]
        if self.model_cfg.BOX_EMBEDDING:
    ...
    def forward(self, batch_dict):
        ...
        merge_gen_out.append(gen_output)
        #############################################

        # 2
        pooled_features, roi_grid_xyz_rel = self.roi_grid_pool(batch_dict, grid_sizes[1])  # (BxN, 6x6x6, C) pooled_features: (B, 6x6x6, 96), roi_grid_xyz_rel(B, 6x6x6, 3)
        merge_pooled_features.append(pooled_features)
        merge_roi_grid_xyz_rel.append(roi_grid_xyz_rel)
        ...
        # Point Generation
        gen_output = self.gen_fc_layers_1(attention_output.view(-1, attention_output.shape[-1]))
        merge_gen_out.append(gen_output)
        #############################################

        gen_output = torch.cat(merge_gen_out, dim=0)
