# 在transformer後concate三種grid size的結果一起丟進去產生點
import torch
...
from ..model_utils.attention_utils import TransformerEncoder, TransformerDecoder, get_positional_encoder, PointFeatureExtractor
...
class PGRCNNHead(RoIHeadTemplatePG):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        # RoI Grid Pooling
        c_out_1 = 0
        self.roi_grid_pool_layers_1 = nn.ModuleList()
        for src_name in self.pool_cfg.FEATURES_SOURCE:
            mlps = LAYER_cfg[src_name].MLPS
            for k in range(len(mlps)):
                mlps[k] = [backbone_channels[src_name]] + mlps[k]
            ...
            c_out_1 += sum([x[-1] for x in mlps])
            
            self.roi_grid_pool_layers_1.append(pool_layer)

        # Transformer Encoder
        assert self.attention_cfg.ENCODER.NUM_FEATURES == c_out, f'ATTENTION.ENCODER.NUM_FEATURES must equal voxel aggregation output dimension of {c_out}.'
        self.pos_encoder_1 = get_positional_encoder(self.attention_cfg)
        self.attention_head_1 = TransformerEncoder(self.attention_cfg.ENCODER)
        
        # RoI Grid Pooling
        c_out_2 = 0
        self.roi_grid_pool_layers_2 = nn.ModuleList()
        for src_name in self.pool_cfg.FEATURES_SOURCE:
            mlps = [[64, 32, 32]]
            for k in range(len(mlps)):
                mlps[k] = [backbone_channels[src_name]] + mlps[k]
            ...
            c_out_2 += sum([x[-1] for x in mlps])
            
            self.roi_grid_pool_layers_2.append(pool_layer)

        # Transformer Encoder
        assert self.attention_cfg.ENCODER.NUM_FEATURES == c_out, f'ATTENTION.ENCODER.NUM_FEATURES must equal voxel aggregation output dimension of {c_out}.'
        self.pos_encoder_2 = get_positional_encoder(self.attention_cfg)
        self.attention_head_2 = TransformerEncoder(self.attention_cfg.ENCODER)

        # Gate
        self.gate_layer = nn.Sequential(
            nn.Linear(self.attention_cfg.ENCODER.NUM_FEATURES, self.attention_cfg.ENCODER.NUM_FEATURES),
            nn.Sigmoid()
        )
    ...
    def roi_grid_pool(self, batch_dict, grid_size):
        """
        Args:
            batch_dict:
                batch_size:
                rois: (B, num_rois, 7 + C) 7:可能是(x, y, z, l(dx), w(dy), h(dz), XX)
        ...
        """
        rois = batch_dict['rois'] # (B, ROI_PER_IMAGE:128, 7) 
    ...
    def roi_grid_pool_1(self, batch_dict, grid_size):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        pooled_features_list = []
        for k, src_name in enumerate(self.pool_cfg.FEATURES_SOURCE):
            pool_layer = self.roi_grid_pool_layers_1[k]
            cur_stride = batch_dict['multi_scale_3d_strides'][src_name]
            cur_sp_tensors = batch_dict['multi_scale_3d_features'][src_name]
    ...
    def roi_grid_pool_2(self, batch_dict, grid_size):
        """
        Args:
            batch_dict:
                batch_size:
        ...
        pooled_features_list = []
        for k, src_name in enumerate(self.pool_cfg.FEATURES_SOURCE):
            pool_layer = self.roi_grid_pool_layers_2[k]
            cur_stride = batch_dict['multi_scale_3d_strides'][src_name]
            cur_sp_tensors = batch_dict['multi_scale_3d_features'][src_name]
    ...
    def get_global_grid_points_of_roi(self, rois, grid_size):
        rois = rois.view(-1, rois.shape[-1]) # (B*ROI_PER_IMAGE, 7)
    ...
    def forward(self, batch_dict):
        ...
        # RoI aware pooling
        grid_sizes = self.pool_cfg.GRID_SIZE
        merge_pooled_features = []
        merge_roi_grid_xyz_rel = []
        merge_attention_output = []
        merge_global_roi_grid_points = []
        
        # 1
        pooled_features, roi_grid_xyz_rel = self.roi_grid_pool(batch_dict, grid_sizes[0])  # (BxN, 6x6x6, C) pooled_features: (B, 6x6x6, 96), roi_grid_xyz_rel(B, 6x6x6, 3)
        merge_pooled_features.append(pooled_features)
        merge_roi_grid_xyz_rel.append(roi_grid_xyz_rel)
        ...
        merge_global_roi_grid_points.append(global_roi_grid_points)
        ...
        merge_attention_output.append(attention_output)
        #############################################
        # 2
        pooled_features, roi_grid_xyz_rel = self.roi_grid_pool_1(batch_dict, grid_sizes[1])  # (BxN, 6x6x6, C) pooled_features: (B, 6x6x6, 96), roi_grid_xyz_rel(B, 6x6x6, 3)
        merge_pooled_features.append(pooled_features)
        merge_roi_grid_xyz_rel.append(roi_grid_xyz_rel)

        # Transformer Encoder
        positional_input, global_roi_grid_points = self.get_positional_input(batch_dict['rois'], batch_dict['roi_labels'], roi_grid_xyz_rel)
        positional_embedding = self.pos_encoder_1(positional_input)
        merge_global_roi_grid_points.append(global_roi_grid_points)
        ...
        attention_output = self.attention_head_1(pooled_features, positional_embedding)
        merge_attention_output.append(attention_output)
        #############################################
        # 3
        pooled_features, roi_grid_xyz_rel = self.roi_grid_pool_2(batch_dict, grid_sizes[2])  # (BxN, 6x6x6, C) pooled_features: (B, 6x6x6, 96), roi_grid_xyz_rel(B, 6x6x6, 3)
        merge_pooled_features.append(pooled_features)
        merge_roi_grid_xyz_rel.append(roi_grid_xyz_rel)

        # Transformer Encoder
        positional_input, global_roi_grid_points = self.get_positional_input(batch_dict['rois'], batch_dict['roi_labels'], roi_grid_xyz_rel)
        positional_embedding = self.pos_encoder_2(positional_input)
        merge_global_roi_grid_points.append(global_roi_grid_points)
        ...
        attention_output = self.attention_head_2(pooled_features, positional_embedding)
        merge_attention_output.append(attention_output)
        #############################################

        pooled_features = torch.cat(merge_pooled_features, dim=1)
        roi_grid_xyz_rel = torch.cat(merge_roi_grid_xyz_rel, dim=1)
        global_roi_grid_points = torch.cat(merge_global_roi_grid_points, dim=1)
        
        # Point Generation
        attention_output = torch.cat(merge_attention_output, dim=1)
        # Gate-based Fusion
        gates = self.gate_layer(attention_output)  # (B, N, C)
        attention_output = gates * attention_output
